...
if($_REQUEST['action']=='makeMeGalThumbnail'){

	$db= new Connection();
	$db->from="galleries_uploads";
	$db->select="*";
	$db->where=array("status"=>1,"gid"=>dataDecode($_REQUEST['productId']),"id"=>dataDecode($_REQUEST['imageId']));
	$db->limit=200;
	$db->type="image data";
	$sno=1;
	$images=$db->get_all_records()['items'][0];
...
if($_REQUEST['action']=='deleteGalleryThumbnail'){
