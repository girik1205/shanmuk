...
<table width="100%" class="table table-striped table-bordered table-hover dataTable">
	...
	<tbody>
		<?php
			$db= new Connection();
			$db->from="services";
			$db->select=" services.*,categories.name as cat_name ";
			$db->where=array("services.status"=>1);
			$db->limit=500;
			$db->type="services data";
			$db->join=" join categories on services.cat_type=categories.id";
			$sno=1;
			$services=$db->get_all_records()['items'];
			$db->connectionClose();
	...
	<div class="tab-content row">
		<div class="tab-pane active" id="tab1">
			<form method="post" class="form-group padding15">
				...
				<input type="text" name="name" id="name" placeholder="Service Name"  class="form-control" required value="<?=$formData['name']?>" pattern="[A-Za-z0-9.\-\s]{2,40}">
				...
				<select name="cat_type" id="cat_type"  class="form-control" required>
					...
					$db->where=array("status"=>1,"cat_type"=>1);
					$db->limit=200;
				...
				<input type="number" name="price" id="price" placeholder="Price"   class="form-control" value="<?=$formData['price']?>" >
				<label for="offerprice">Offer Price</label>
				<input type="number" name="offerprice" id="offerprice" placeholder="Offer Price"  class="form-control"  value="<?=$formData['offerprice']?>" >
				<label for="stockcount">Stock Count</label>
				<input type="number" name="stockcount" id="stockcount" placeholder="Stock Count"  class="form-control"  value="<?=$formData['stockcount']?>" > -->
		...
		<div class="tab-pane" id="tab2">
			<form method="post" class="form-group padding15">
				...
				<input type="text" name="seo_name" id="seo_name" placeholder="Seo Name"  class="form-control" required value="<?=$formData['seo_name']?>" pattern="[A-Za-z0-9.,\-\s]{2,100}">
				...
				<label for="seo_keywords">Seo Description</label>
		...
		<div class="tab-pane" id="tab3">
			...
			$db->select="*";
			$db->where=array("status"=>1,"service_id"=>$formData['id']);
			$db->limit=200;
			$db->type="image data";
			$sno=1;
			...
			foreach($images as $key => $value){ ?>
				<div class="col-xs-3">
					<div class="thumbnail">
						...
						<input type="checkbox" id="<?=$value['id'].'-'.$value['service_id']?>"  name="makethumbnail" value="<?=dataEncode($value['id']).'#'.dataEncode($value['service_id'])?>" class="makethumbnail"> <label class="thumbnailText" for="<?=$value['id'].'-'.$value['service_id']?>"> Make This As Thumbnail </label>
						<?php } ?>
					</div>
				</div>
			<?php } ?>
    ...
    if($(this).is(":checked")) {
    	var postData = $(this).val().split('#');
	   	$.ajax({
			type: "POST",
			url: '<?=$location?>',
			data: {"productId":postData[1],"imageId":postData[0],"action":"makeMeThumbnail_service"},
			success: function( result, textStatus){
				if(result=='success'){
					location.reload();
				}
