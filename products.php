<?php
if(substr( $_REQUEST['product'], 0, 2 ) === "_p"){
    $_REQUEST['p_page']=substr( $_REQUEST['product'],2);
    $_REQUEST['product']='';
} else if(substr( $_REQUEST['product'], 0, 3 ) === "_s="){
    $_REQUEST['product_search']=substr( $_REQUEST['product'],3);
    $_REQUEST['product']='';
}
if(substr( $_REQUEST['search'], 0, 3 ) === "_s="){
    $_REQUEST['product_search']=substr( $_REQUEST['search'],3);
}
...
if($_REQUEST['product_search']!=''){
    $db->whereLike=array("products.name"=>'%'.$_REQUEST['product_search'].'%');
}
//$db->limit=300;
$db->limit=$pagg_limit;
...
$itemsCnt=ceil($result['totalCount']/$pagg_limit);
...
?>
            ...
            <br>
                <nav aria-label="...">
                  <ul class="pagination pagination-sm">
                    ...
                    <form name="ps" action="<?=$clocation?>" method="post">
                        <input type="text"  required name="product_search" id="product_search" value="<?=$_REQUEST['product_search']?>" >
            ...
            <br>
                <nav aria-label="...">
                  <ul class="pagination pagination-sm">
                    <li class="page-item <?=$_REQUEST['p_page']==0?'disabled':'';?>">
                      <a class="page-link" href="<?=$pageUrl=$clocation.'';?>" tabindex="-1"><<</a>
                    </li>
                    <?php for($page=0;$page<$itemsCnt;$page++){
                        /*if($page==0)
                        $pageUrl=$clocation;
                        else*/
                        $pageUrl=$clocation.'/_p'.$page;
                        if($_REQUEST['product_search'])
                            $pageUrl.='/_s='.$_REQUEST['product_search'];
                        ?> 
                    ...
                    <li class="page-item <?=$_REQUEST['p_page']==$itemsCnt-1?'disabled':'';?>">
                      <a class="page-link" href="<?=$pageUrl=$clocation.'/_p'.($itemsCnt-1);?>">>></a>
                    </li>
                  </ul>
                </nav>
            <br>
            ...
            <div class="clearfix"></div>
            <div class="col-md-12 com-xs-12 hidden">
            <div class="pull-right" >
            <br>
                <nav aria-label="...">
                  <ul class="pagination pagination-sm">
